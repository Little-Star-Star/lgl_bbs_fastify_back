...
exports.post_replyComment = async (req, reply) => {
    ...
    try {
        ...
        if (!req.userLogin || !userId) {
            ...
            const r = await model_newsComment.findOneAndUpdate({
                _id: newsCommentId
            }, {
                '$push': {
                    replyData: {
...
/**
 * 获取某一二手物品的所有留言信息
 * /secondHand/comment/list/:secondHandId
 * @param {*} req 
 * @param {*} reply
 */
exports.get_secondHandCommentList = async (req, reply) => {
    try {
        let {
            secondHandId
        } = req.params
        const r = await model_secondHandComment.findOne({
...
/**
 * 发表对于二手商品的留言
 * /secondHand/release/comment
 * @param {*} req 
 * @param {*} reply
 */
exports.post_secondHandReleaseComment = async (req, reply) => {
    const userId = req.session ? req.session.userId : ''
    try {
        let {
            secondHandId,
        ...
        if (!req.userLogin || !userId) {
            ...
            const r = await model_secondHandComment.findOneAndUpdate({
                secondHand: secondHandId
            }, {
                $push: {
                    'comments': {
